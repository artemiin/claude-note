...
def _memory_lock(memory_path: Path, timeout: float = 30.0):
    ...
    Uses the same file locking pattern as managed_blocks.py.
    """
    import contextlib

    @contextlib.contextmanager
    def _lock():
        lock_dir = memory_path.parent
        lock_dir.mkdir(parents=True, exist_ok=True)

        path_hash = hashlib.sha256(str(memory_path).encode()).hexdigest()[:16]
        lock_file = lock_dir / f".memory-{path_hash}.lock"
        ...
        try:
            ...
            if not acquired:
                raise TimeoutError(f"Could not acquire lock for {memory_path}")
